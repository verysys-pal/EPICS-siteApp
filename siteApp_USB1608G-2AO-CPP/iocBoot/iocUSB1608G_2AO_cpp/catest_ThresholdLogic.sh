...
# ThresholdLogic 컨트롤러 정의 (ai1->bo5, ai2->bo6, ai4->bo7, ai5->bo8)
...
# PV 생성 함수
function get_threshold_pv() { echo "${PREFIX}${1}Threshold"; }
function get_hysteresis_pv() { echo "${PREFIX}${1}Hysteresis"; }
function get_current_value_pv() { echo "${PREFIX}${1}CurrentValue"; }
function get_output_state_pv() { echo "${PREFIX}${1}OutputState"; }
function get_enable_pv() { echo "${PREFIX}${1}Enable"; }
function get_update_rate_pv() { echo "${PREFIX}${1}UpdateRate"; }
function get_alarm_state_pv() { echo "${PREFIX}${1}AlarmState"; }
function get_reset_pv() { echo "${PREFIX}${1}Reset"; }

# 입력/출력 채널 PV 생성 함수
function get_input_pv() {
    local controller=$1
    local channels=${CONTROLLERS[$controller]}
    local input_channel=${channels%:*}
    echo "${PREFIX}${input_channel}"
}

function get_output_pv() {
    local controller=$1
    local channels=${CONTROLLERS[$controller]}
    local output_channel=${channels#*:}
    echo "${PREFIX}${output_channel}"
}

# 기본 PV들 (ThresholdLogic1용)
THRESHOLD_PV=$(get_threshold_pv "$CONTROLLER")
HYSTERESIS_PV=$(get_hysteresis_pv "$CONTROLLER")
CURRENT_VALUE_PV=$(get_current_value_pv "$CONTROLLER")
OUTPUT_STATE_PV=$(get_output_state_pv "$CONTROLLER")
ENABLE_PV=$(get_enable_pv "$CONTROLLER")
UPDATE_RATE_PV=$(get_update_rate_pv "$CONTROLLER")
ALARM_STATE_PV=$(get_alarm_state_pv "$CONTROLLER")
RESET_PV=$(get_reset_pv "$CONTROLLER")

# 입력 시뮬레이션용 아날로그 출력 (Ao1으로 Ai1 시뮬레이션)
AO_PV="${PREFIX}Ao1"
...
function test_pv_connections() {
    ...
    print_section "모든 ThresholdLogic 컨트롤러 PV 연결 확인"
    
    local failed=0
    
    # 각 ThresholdLogic 컨트롤러별 PV 테스트
    for controller in "${!CONTROLLERS[@]}"; do
        print_info "테스트 중: $controller (${CONTROLLERS[$controller]})"
        
        local pvs=(
            "$(get_threshold_pv "$controller")"
            "$(get_hysteresis_pv "$controller")"
            "$(get_current_value_pv "$controller")"
            "$(get_output_state_pv "$controller")"
            "$(get_enable_pv "$controller")"
            "$(get_update_rate_pv "$controller")"
            "$(get_alarm_state_pv "$controller")"
            "$(get_input_pv "$controller")"
            "$(get_output_pv "$controller")"
        )
    ...
    # 아날로그 출력 PV 테스트
    print_info "테스트용 아날로그 출력 PV 확인"
    if ! check_pv_connection "$AO_PV"; then
        ((failed++))
    fi
