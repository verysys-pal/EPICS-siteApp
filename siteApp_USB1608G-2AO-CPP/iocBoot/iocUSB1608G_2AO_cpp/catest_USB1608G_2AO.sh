...
function set_automode() {
  ...
  caput USB1608G_2AO_cpp:AiMode 1
  for i in {5..8}; do
    caput USB1608G_2AO_cpp:Threshold:Ai1-Bo$i:AutoMode 1
  done
  sleep 2
}
...
function set_voltage_and_read() {
  ...
  echo ""
  echo "--------- Ao1 = ${voltage}V"
  caput USB1608G_2AO_cpp:Ao1 "$voltage"
  sleep 3
  caget USB1608G_2AO_cpp:Ai1
  for i in {5..8}; do
    caget USB1608G_2AO_cpp:Threshold:Ai1-Bo$i:Calc
  done
  for i in {5..8}; do
    caget USB1608G_2AO_cpp:Bo$i
  done
}
...
function manual_calc_test() {
  ...
  # CALC, OUT 필드 수동 설정
  caput USB1608G_2AO_cpp:Threshold:Ai1-Bo6:Calc.CALC "B?(A>=2):C"
  caput USB1608G_2AO_cpp:Threshold:Ai1-Bo6:Calc.OUT "USB1608G_2AO_cpp:Bo6 PP"

  # 테스트 전압 설정
  caput -t USB1608G_2AO_cpp:Ao1 2.5
  sleep 3
  caget USB1608G_2AO_cpp:Ai1

  # PROC 강제 실행
  caput USB1608G_2AO_cpp:Threshold:Ai1-Bo6:Calc.PROC 1

  # 결과 확인
  caget USB1608G_2AO_cpp:Threshold:Ai1-Bo6:Calc
  caget USB1608G_2AO_cpp:Bo5
  caget USB1608G_2AO_cpp:Bo6

  # 다시 0V로 설정
  caput -t USB1608G_2AO_cpp:Ao1 0
  sleep 3
  caget USB1608G_2AO_cpp:Ai1
}
...
function sine_wave_ao1() {
  ...
  for ((i = 0; i < samples; i++)); do
    ...
    # 출력
    printf "[%03d] Ao1 = %.3f V\n" "$i" "$voltage"
    caput -t USB1608G_2AO_cpp:Ao1 "$voltage"
